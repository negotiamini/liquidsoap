  ...
  let parse_param label value =
    match label with
      ...
      | "pixel_aspect" ->
          let pixel_aspect =
            try
              let rex = Pcre_compat.regexp "(\\d+)/(\\d+)" in
              let sub = Pcre_compat.exec ~rex value in
              Some
                {
                  Avutil.num = int_of_string (Pcre_compat.get_substring sub 1);
                  den = int_of_string (Pcre_compat.get_substring sub 2);
                }
            with _ -> None
          in
          Some
