...
module Make (T : Transport_t) : T with type socket = T.socket = struct
  ...
  let parse_icy_request_line ~port h r =
    let auth_data = Pcre_compat.split ~pat:":" r in
  ...
  let parse_http_request_line r =
    try
      let data = Pcre_compat.split ~rex:(Pcre_compat.regexp "[ \t]+") r in
  ...
  let parse_headers headers =
    let split_header h l =
      try
        let rex = Pcre_compat.regexp "([^:\\r\\n]+):\\s*([^\\r\\n]+)" in
        let sub = Pcre_compat.exec ~rex h in
        (Pcre_compat.get_substring sub 1, Pcre_compat.get_substring sub 2) :: l
      with Not_found -> l
    in
    ...
    try
      let user, pass =
        try
          ...
          let data =
            Pcre_compat.split ~rex:(Pcre_compat.regexp "[ \t]+") auth
          in
          match data with
            | "Basic" :: x :: _ -> (
                let auth_data = Pcre_compat.split ~pat:":" (Utils.decode64 x) in
                match auth_data with
                  | x :: y :: _ -> (x, y)
                  | _ -> raise Not_found)
            | _ -> raise Not_found
  ...
  let handle_http_request ~hmethod ~hprotocol ~data ~port h uri headers =
    ...
    let f (verb, reg_uri) handler =
      let rex = Pcre_compat.regexp reg_uri in
      if (verb :> verb) = hmethod && Pcre_compat.pmatch ~rex uri then (
  ...
  let handle_client ~port ~icy h =
    ...
    in
    Duppy.Monad.bind __pa_duppy_0 (fun s ->
        let lines = Pcre_compat.split ~rex:(Pcre_compat.regexp "[\r]?\n") s in
        let __pa_duppy_0 =
